...
try {
    ...
    // Silent upgrade for existing databases
    try {
...
// Release session lock to prevent lag when polling
session_write_close(); 
...
// Transfer Logic Engine (Moved out of the transaction block)
    ...
    foreach($offer as $item) {
        if (strpos($item, 'item::') === 0) { 
            ...
            if ($idx !== false) {
                ...
                throw new Exception("Security Abort: Player attempted to trade an item they do not own.");
            }
            ...
            if ($idx !== false) {
                ...
                // Unequip if traded away
                if ($from_active_pet === $item) $from_active_pet = '';
            } else {
                throw new Exception("Security Abort: Player attempted to trade a pet they do not own.");
            }
        } else { 
            // Cursor
            ...
            if ($idx !== false) {
                ...
                throw new Exception("Security Abort: Player attempted to trade a cursor they do not own.");
            }
        }
    }
}
...
if ($action === 'update_offer') {
    $code = $_POST['room_code'] ?? '';
    $offer = $_POST['offer'] ?? '[]';
    $gems = max(0, (int)($_POST['gems'] ?? 0)); // Prevent negative gem offers
    $pet_ages = $_POST['pet_ages'] ?? '{}'; 
    
    $room = getRoom($pdo, $code);
    if (!$room) die(json_encode(["success" => false]));

    // Check valid JSON structure before saving to prevent DB corruption
    if (json_decode($offer) === null) $offer = '[]';
    if (json_decode($pet_ages) === null) $pet_ages = '{}';
...
if ($action === 'toggle_accept') {
    ...
    if ($p1_ready && $p2_ready && $room['status'] !== 'completed') {
        try {
            ...
            // Security Check: Verify users have enough gems
            if ($p1['gems'] < $p1_gems_offer || $p2['gems'] < $p2_gems_offer) {
                throw new Exception("Security Abort: Insufficient gems for trade.");
            }
            ...
            // Swap the items (Throws Exception and aborts if fake items are injected)
            ...
            // Swap the Gems
            $p1_final_gems = ($p1['gems'] - $p1_gems_offer) + $p2_gems_offer;
            $p2_final_gems = ($p2['gems'] - $p2_gems_offer) + $p1_gems_offer;
            ...
            $pdo->rollBack();
            // Optional: You could log $e->getMessage() here for security auditing
        }
    }
...
if ($action === 'sync') {
    ...
    // Fixed: properly decode the stored JSON so it is serialized correctly in the final response
    $their_pet_ages_raw = $is_p1 ? ($room['p2_pet_ages'] ?? '{}') : ($room['p1_pet_ages'] ?? '{}'); 
    $their_pet_ages = json_decode($their_pet_ages_raw, true) ?: []; 
